...
# ---------------------------------------------------------------------------
box::use(
  data.table[.N, as.data.table, data.table, setorder],
...
stem_ba_ft2 <- function(trees) {
  ...
  ba <- if (!is.null(trees$BasalA)) {
    suppressWarnings(as.numeric(trees$BasalA))
  } else {
...
scaling_area_m2 <- function(nonocarea, radius_m, mode = c("nonoc", "full")) {
  ...
  if (length(a) != 1 || is.na(a) || a <= 0 || a > full * 1.05) {
    return(full)
  }
  a
}
...
stand_metrics <- function(trees, area_m2) {
  ...
  if (n == 0 || is.na(area_m2) || area_m2 <= 0) {
    return(list(
      n = 0L, tpa = NA_real_, tph = NA_real_, ba_ac = NA_real_,
  ...
  n <- length(dbh_in)
  if (n == 0) {
    return(stand_metrics(trees[0], area_m2))
  }
...
detection_shortfall <- function(dbh_in, visible_fraction = 1, class_width = 2) {
  ...
  tab <- data.table(cls = cls)[, list(detected = .N), by = cls]
  setorder(tab, cls)
  p <- detection_prob(tab$cls + class_width/2, visible_fraction)
  tab[, inferred_total := detected/p]
  tab[, missing := pmax(0, inferred_total - detected)] # nolint: unused_declared_object_linter.
  tab[]
}
...
assign_species <- function(trees, species, rule = c("random", "size", "cluster"),
  ...
  sp_pct <- suppressWarnings(as.numeric(sp$pct))
  sp <- sp[!is.na(code) & nzchar(as.character(code)) & sp_pct > 0]
  if (nrow(sp) == 0) {
  ...
  if (rule == "random") {
    ...
    idx[idx < 1] <- 1L
    idx[idx > length(codes)] <- length(codes)
    return(codes[idx])
  }
  ...
  ord <- switch(rule,
    ...
    cluster = order(atan2(
      suppressWarnings(as.numeric(trees$Y)),
...
mark_removals <- function(trees, area_m2,
  ...
  if (method == "species") {
    if (is.null(remove_species) || !"species" %in% names(trees)) {
      return(rm_flag)
    }
...
fit_weibull <- function(dbh_in) {
  d <- dbh_in[!is.na(dbh_in) & dbh_in > 0]
  if (length(d) < 5) {
    return(list(shape = 2, scale = max(4, mean(d, na.rm = TRUE))))
  }
  m <- mean(d)
  s <- sd(d)
  if (!is.finite(s) || s <= 0) {
    return(list(shape = 3, scale = m))
  }
  cv <- s/m
...
clark_evans <- function(x, y, area_m2) {
  ...
  y <- y[ok]
  n <- length(x)
  if (n < 3 || is.na(area_m2) || area_m2 <= 0) {
    return(NA_real_)
  }
...
simulate_stems <- function(width_m, height_m, density_tph, shape, scale,
  ...
  if (pattern == "cluster") {
    ...
    px <- runif(n_par, 0, width_m)
    py <- runif(n_par, 0, height_m)
    xs <- numeric(0)
    ys <- numeric(0)
    ...
    xs <- runif(n_target, 0, width_m)
    ys <- runif(n_target, 0, height_m)
    b <- band_of(xs)
    thin <- runif(length(xs)) <= strata$rel[b]
    xs <- xs[thin]
    ys <- ys[thin]
  ...
  d_cm <- dbh * IN_TO_CM
  ht_denom <- d_cm + 16
  ht <- 1.37 + 28 * d_cm/ht_denom * runif(n, 0.85, 1.15) # metres
