...
brown_class_load <- function(count, cls, n_planes = 1,
  ...
  # The count comes straight off a numericInput that renderUI creates, so on
  # the first flush - before the browser has registered those inputs - it
  # arrives as NULL. as.numeric(NULL) is numeric(0), and `logical(0) || ...`
  # evaluates to NA, which makes the `if` throw rather than return NA. Check
  # the length first so an absent input reads as "no value", like a blank box.
  count <- suppressWarnings(as.numeric(count))
  if (length(count) != 1 || is.na(count) || count < 0) {
    return(NA_real_)
  }
