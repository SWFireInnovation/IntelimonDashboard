...
build_treeinit <- function(trees, stand_id, ef_tpa,
  ...
  has_increments <- !is.null(increments) && nrow(increments) > 0 &&
...
build_keyfile <- function(stand_id, inv_year, area_ac, variant,
  ...
  if (!is.null(thinning)) {
    ...
    add(
      thinning$keyword, "       ", format(thinning$year), "  ",
...
thin_keyword <- function(method, target_ba_ac = NULL, species = NULL,
  ...
  switch(method,
    below = list(
      keyword = "ThinBBA", year = year, value = target_ba_ac,
      comment = "thin from below to residual basal area"
    ),
    above = list(
      keyword = "ThinABA", year = year, value = target_ba_ac,
      comment = "thin from above to residual basal area"
    ),
    species = list(
      keyword = "ThinDBH", year = year, value = species,
      comment = "remove species component"
    ),
    spacing = list(
      keyword = "ThinSDI", year = year, value = target_ba_ac,
